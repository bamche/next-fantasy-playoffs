...
export default async function playerStats(req, res) {
  ...
  try{
    ...
    const teamViewData = await GetTeamView(email);

    console.log("teamViewData", teamViewData[0].teamviewstats.players);
    // console.log("teamViewStats", teamViewStats);
    const teamViewStats = [...teamViewData[0].teamviewstats.players, { name: "Total Team Score", id: 12, total_score: teamViewData[0].teamviewstats.total_score }];
    // redisClient.set(`team-view-${email}`, JSON.stringify(teamViewData));
